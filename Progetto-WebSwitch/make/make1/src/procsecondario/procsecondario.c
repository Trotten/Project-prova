...
//funzione che  viene eseguita dal thread del processo figlio per rimanere in ascolto in lettura sulla pipe di
...
void *gestorenumero(void *arg){								//funzione che crea ed esegue il 
											//thread gestore della creazione di 
											//ulteriori thread liberi, se sono
											//troppo pochi. Non li killa perché 
											//i thread dopo un tot di tempo si 
											//suicidano (da vedere bene per 
											//accertarsi di questo fatto)
...
pthread_t creathreadpergestionepipepadre(struct datiprocfiglio *data){		//funzione per la creazione del
										//thread (del processo secondario) in
										//ascolto sulla pipe con il processo 
										//principale
...
void lavproc(struct datiprocfiglio *datiproc){		//pipe 0 lettura 3 scrittura 
	...
	if(close(info->pip[1])!=0)			//chiusura delle pipe che non servono
		perror("Close");
